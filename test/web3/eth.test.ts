...
describe('web3.eth', () => {
    ...
    it('sendTransaction with valid response', (done) => {
        web3.eth.sendTransaction({from: 0}).then(() => {
            ...
            try {
                expect(() => { throw e || 'no error' }).to.throw('[meter-provider]The private key corresponding to from filed can\'t be found in local eth.accounts.wallet!')
                done()
            } catch (err) {
                done(err)
            }
...
describe('web3.eth:error handling', () => {
    ...
    it('execute method with response code 0(means server is not responding) throw error', (done) => {
        ...
        web3.eth.getBlock(1).then(() => {
            ...
            try {
                expect(() => { throw e || 'no error' }).to.throw('[meter-provider] Invalid response, check the host')
                done()
            } catch (err) {
                done(err)
            }
    ...
    it('execute method with invalid response code and no response body should throw error', (done) => {
        ...
        web3.eth.getBlock(1).then(() => {
            ...
            try {
                expect(() => { throw e || 'no error' }).to.throw('[meter-provider] Invalid response code from provider: 500')
                done()
            } catch (err) {
                done(err)
            }
