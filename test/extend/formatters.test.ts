...
const meterTxReceiptWithUnsafeFields = {
    gasUsed: 55365,
    paid: '0x7aef5bddbe52000',
    reward: '0x24e1685c1f7f000',
    reverted: false,
...
Object.defineProperty(meterTx, 'isMeterified', { get: () => true, set: () => null })
Object.defineProperty(meterTxWithNullTo, 'isMeterified', { get: () => true, set: () => null })
Object.defineProperty(meterTxWithUnsafeFields, 'isMeterified', { get: () => true, set: () => null })
Object.defineProperty(meterTxReceipt, 'isMeterified', { get: () => true, set: () => null })
Object.defineProperty(meterTxReceiptWithUnsafeFields, 'isMeterified', { get: () => true, set: () => null })
...
describe('web3 formatters', () => {
    ...
    it('Format meter transaction', () => {
        const tx = web3.extend.formatters.outputTransactionFormatter(meterTx)
        expect(tx).to.have.all.keys('block', 'blockRef', 'chainTag', 'clauses', 'dependsOn', 'expiration', 'gas', 'gasPriceCoef', 'id', 'nonce', 'origin', 'size')
    })

    it('Format meter transaction with null to', () => {
        const tx = web3.extend.formatters.outputTransactionFormatter(meterTxWithNullTo)
        expect(tx).to.have.all.keys('block', 'blockRef', 'chainTag', 'clauses', 'dependsOn', 'expiration', 'gas', 'gasPriceCoef', 'id', 'nonce', 'origin', 'size')
    })

    it('Format meter transaction with unsafe input', () => {
        const tx = web3.extend.formatters.outputTransactionFormatter(meterTxWithUnsafeFields)
    ...
    it('Format meter transaction receipt', () => {
        const tx = web3.extend.formatters.outputTransactionReceiptFormatter(meterTxReceipt)
        expect(tx).to.have.all.keys('meta', 'gasPayer', 'gasUsed', 'outputs', 'paid', 'reverted', 'reward')
    })

    it('Format meter transaction receipt with unsafe input', () => {
        const tx = web3.extend.formatters.outputTransactionReceiptFormatter(meterTxReceiptWithUnsafeFields)
        expect(tx).to.have.all.keys('gasUsed', 'outputs', 'paid', 'reverted', 'reward', 'meta')
    })

    it('Format meter transaction receipt with eth receipt properties', () => {
        Object.defineProperty(meterTxReceipt, 'transactionIndex', { value: 1, configurable: true })
        Object.defineProperty(meterTxReceipt, 'cumulativeGasUsed', { value: 1000000, configurable: true })
        const tx = web3.extend.formatters.outputTransactionReceiptFormatter(meterTxReceipt)
    ...
    it('Format meter log', () => {
        const log = meterTxReceipt.outputs[1].events[0]
        if (!log.isMeterified) { Object.defineProperty(log, 'isMeterified', { get: () => true }) }
